...
def get_folder_name() -> str:
    ...
    return f"data/raw/calendar/index"
...
def prepare_inputs(ticker, expirations, existing_files, gaps=4):
    ...
    fexps = [
        d
        for d in get_expiry_dates(ticker)
        if d < int((dt.datetime.now() - dt.timedelta(days=5)).strftime("%Y%m%d"))
    ]

    first_dates = pd.Timestamp("2016-01-01").strftime("%Y%m%d")  # Standard subscription
    fexps = [d for d in fexps if d > int(first_dates)]

    for en, date in enumerate(fexps):
        kwargs = {
            "symbol": ticker,
            # "right": right,
            "fexp": date,
            "bexps": sorted([d for d in expirations if d > date])[:gaps],
            "roots": [ticker],
            "filepath": f"{get_folder_name()}/{ticker}/{date}.parquet",
            "ivl": ivl,
        }
...
if __name__ == "__main__":
    ...
    bucket = S3Handler(bucket_name=os.getenv("S3_BUCKET_NAME"), region="us-east-2")
    existing_files = bucket.list_files(f"{get_folder_name()}")
    inputs = prepare_inputs(ticker=ticker, expirations=expirations, existing_files=existing_files)

    failed_files_path = f"data/raw/{ticker}_calendar/failed_files.parquet"
