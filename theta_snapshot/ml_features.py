...
from theta_snapshot import read_from_db, calculate_buisness_days
import time
...
def histroical_grade(df, latest=False):
    ...
    if latest:
        ...
        duplicated_columns = [c for c in grades.columns if c in df.columns and c not in ["symbol", "weeks"]]
...
def generate_predictions(theta_df):
    try:
        ...
        preds = pd.DataFrame(ml_pipe.predict_proba(pred_df)).rename(
            columns={0: "prediction_score_0", 1: "prediction_score_1"}
        )
        preds["prediction_label"] = preds["prediction_score_1"].apply(lambda x: 1 if x > 0.85 else 0)
        ...
        theta_df = theta_df.merge(
            ...
            on=["symbol", "strike", "right", "exp_front", "weeks"],
            how="left",
        )
...
def price_trend(symbols):
    df = read_from_db(
        query=f"""SELECT "symbol", "weeks", "date", "calCostPct", "reportDate", "ask_size_front","bid_size_back","ask_size_back", "bid_size_front"  FROM public."ThetaSnapshot" WHERE symbol in {tuple(symbols)};"""
    )
    ...
    df = (
        df.groupby(["bdte", "symbol", "weeks"])
        .agg(
            stddev=("calCostPct", "std"),
            max=("calCostPct", "max"),
            min=("calCostPct", "min"),
            avg=("calCostPct", "mean"),
            count=("calCostPct", "count"),
            min_entry_size_mean=("min_entry_size", "mean"),
            max_entry_size_mean=("max_entry_size", "mean"),
            min_exit_size_mean=("min_exit_size", "mean"),
            max_exit_size_mean=("max_exit_size", "mean"),
        )
        .reset_index()
    )
...
def trend_slicer(pred_df, trend_df):
    ...
    for idx, row in pred_df[["symbol", "bdte", "weeks"]].drop_duplicates().iterrows():
        ...
        if len(pre_slice) >= 3:
            if all([True if x in pre_slice["bdte"].tolist() else False for x in bdtes[1:]]) or all(
                [True if x in pre_slice["bdte"].tolist() else False for x in bdtes[:-1]]
            ):
